def inventory_pickup(inventory: list, items: list, capacity: int) -> list:
    ...
    if len(inventory)>capacity:
        del inventory[:len(inventory)-capacity]
    return inventory
...
def berry_finder(t):
    ...
    if label(t)=='berry':
        return True
    berry_bool=[berry_finder(b) for b in branches(t)]
    return any(berry_bool)
...
def size_of_tree(t):
    ...
    if is_leaf(t):
        return 1
    size_branch=[size_of_tree(b) for b in branches(t)]
    return 1+sum(size_branch)
