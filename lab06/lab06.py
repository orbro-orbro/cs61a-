...
class BankAccount:
    ...
    def deposit(self, amount: int) -> int:
        ...
        """
        self.transactions.append(Transaction(self.count,self.balance,self.balance+amount))
    ...
    def withdraw(self, amount: int) -> int | str:
        ...
        if amount > self.balance:
            self.transactions.append(Transaction(self.count,self.balance,self.balance))
            self.count+=1
...
class VirFib():
    ...
    def next(self):
        if self.pre==-1:
            self.pre=1
        new_instance=VirFib(self.pre+self.value)
        new_instance.pre=self.value
        return new_instance
