...
def closer_city(lat, lon, city_a, city_b):
    ...
    distance_a=distance(target,city_a)
    distance_b=distance(target,city_b)
...
def balanced(t):
    ...
    if is_leaf(t):
        return True 
    branch_sum=[sum_tree(b) for b in branches(t)]
    first_sum=branch_sum[0]
    branch_bool=[bs==first_sum for bs in branch_sum]
    if not all(branch_bool):
        return False
    branch_balanced=[balanced(b) for b in branches(t)]
    return all([balanced(b) for b in branches(t)])
    # 精简版：
    # if is_leaf(t):
    #     return True
    # # set()：自动去重，[3,3,3]→{3} len=1，[2,3,3]→{2,3} len=2
