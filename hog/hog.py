...
def boar_brawl(player_score, opponent_score):
    ...
    if opponent_tens==player_ones:
        return 1
    else:
        return 3*abs(opponent_tens-player_ones)
    # END PROBLEM 2
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    factor=num_factors(score)
    if factor==3 or factor==4:
        new_score=score+1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while player_score<GOAL:
        ...
        while opponent_score<GOAL:
            if strategy(player_score,opponent_score)!=num_rolls:
                return False
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    """
    # BEGIN PROBLEM 9
    roll_averaged=make_averaged(roll_dice,times_called)
    roll_max=1
    aver_max=roll_averaged(1,dice)
    i=2
    while i<=10:
        if aver_max<roll_averaged(i,dice):
            roll_max=i
            aver_max=roll_averaged(i,dice)
        i=i+1
    return roll_max
    # END PROBLEM 9
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    now_score=sus_update(0,score,opponent_score)
    if now_score>=threshold+score:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11


sus_update_aver=make_averaged(sus_update)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    roll_max=1
    threshold=sus_update_aver(1,score,opponent_score)-score
    i=2
    while i<=10:
        if threshold<=sus_update_aver(i,score,opponent_score)-score:
            threshold=sus_update_aver(i,score,opponent_score)-score
            roll_max=i
        i=i+1
    return sus_strategy(score,opponent_score,threshold,roll_max)
